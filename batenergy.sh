...
read online < /sys/class/power_supply/AC0/online
...
case $state in
"pre")
	echo "Batenergy: Saving time and battery energy before sleeping ($sleep_type)."
	echo $now > $FILE
...
"post")
	...
	echo "Batenergy: Duration of $days days $hours hours $minutes minutes sleeping ($sleep_type)."
	...
	echo "Batenergy: Battery energy change of $energy_diff_pct % ($energy_diff mWh) at an average rate of $avg_rate_pct %/h ($avg_rate mW)."
	;;
esac
